...
TAG_MAP = {
    ...
    "audit": "Plant Audits",
    "assessment": "Plant Audits",
    "root cause": "Plant Audits",
    "cost": "Cost & Economics",
    "price": "Cost & Economics",
    "payback": "Cost & Economics",
    "downtime": "Reliability",
    "outage": "Reliability",
    "trip": "Reliability",
    "spare": "Spares & Procurement",
    "procurement": "Spares & Procurement",
    "tender": "Spares & Procurement",
    "specification": "Spares & Procurement",
...
def make_nav_title(title):
    ...
    nav = re.sub(r'\s*[-–—]\s*Axiom Power Services.*$', '', nav)
    nav = re.sub(r'\s*\(20\d{2}.*?\)', '', nav)
    nav = re.sub(r'\s*Lagos:?\s*', ' ', nav).strip()
    if len(nav) <= 30:
        return nav
    words = nav.split()
    result = ''
...
def clean_body_links(body):
    body = re.sub(r'https?://axiompower\.com\.ng/', '/', body)
...
def convert_and_write(draft_path, output_path, publish_date, slug):
    ...
    canonical = f'https://axiompower.com.ng/blog/{slug}/'
